...
function App() {
  ...
  useEffect(() => {
    ...
    // === PERBAIKAN 1: LOAD KERANJANG DARI LOCALSTORAGE ===
  ...
  // === PERBAIKAN 2: SIMPAN KERANJANG KE LOCALSTORAGE SETIAP KALI BERUBAH ===
  useEffect(() => {
    localStorage.setItem('myCart', JSON.stringify(cart));
  }, [cart]); // Dependency array berisi [cart], artinya jalankan ini setiap cart berubah
  ...
  const handleCheckout = async () => {
    ...
    // === PERBAIKAN 3: KOSONGKAN LOCALSTORAGE JUGA SAAT CHECKOUT ===
    setCart([]); 
    localStorage.removeItem('myCart'); // Hapus data keranjang dari storage agar bersih
    setPaymentMethod('');
    ...
    if (paymentMethod === 'Transfer Bank') {
      setInfoModal({
        ...
        content: (
          <div className="modal-text-content">
            <p>Silakan transfer <strong>Rp {totalAmount.toLocaleString('id-ID')}</strong> ke:</p>
            <div className="bank-account"><p>BCA: <strong>1234567890</strong></p><p>a.n. SoleMates</p></div>
            <p className="modal-note">Jangan lupa klik "Saya Sudah Bayar" setelah transfer.</p>
          </div>
        )
      });
      ...
      setInfoModal({
        ...
        content: (
          <div className="modal-center-content">
            ...
            <h3 className="modal-total">Total: Rp {totalAmount.toLocaleString('id-ID')}</h3>
            <p className="modal-note">Pesanan Anda akan diproses setelah kami verifikasi pembayaran Anda.</p>
          </div>
        )
      });
      ...
      setInfoModal({
        ...
        content: (
          <div className="modal-text-content">
            <p>Mohon siapkan uang pas sebesar <strong>Rp {totalAmount.toLocaleString('id-ID')}</strong> saat kurir datang.</p>
          </div>
        )
      });
    }
  ...
  const handleCustomerConfirmPayment = (id) => {
    ...
    showNotification("Konfirmasi pembayaran terkirim! Tunggu admin verifikasi pembayaran Anda.", "success");
  };
    ...
    <div className="container">
      ...
      {isAdmin && (
        <div className="admin-top-bar">
          ...
          <button className="logout-btn-small" onClick={handleLogout}>Keluar</button>
        </div>
      )}
      ...
      <main>
                  ...
                  {cart.map(item => (
                    <div key={item.cartItemId} className="cart-item">
                      <div className="cart-item-info">
                        <span>{item.name} <strong>(Uk. {item.size})</strong></span>
                        <span className="cart-item-qty">x{item.quantity} @ Rp {item.price.toLocaleString('id-ID')}</span>
                      </div>
        ...
        <section className="order-list-section">
          <h2 className="section-title">
            {isAdmin ? '📋 Manajemen Pesanan Masuk' : 'Daftar Pesanan Saya'}
          </h2>
          
          {orders.length === 0 ? <p>Belum ada data pesanan.</p> : (
                ...
                <div key={order.id} className={`order-card ${isAdmin ? 'admin-view' : ''}`}>
                  <div className="order-header">
                    ...
                    <span className={`order-status ${
                      ...
                      order.status === 'Menunggu Verifikasi' ? 'verifying' :
                      order.status === 'Selesai' ? 'completed' : 
                      order.status.includes('COD') ? 'cod' : 'processing'
                    }`}>
                      {order.status}
                    </span>
                  </div>
                  ...
                  <div className="order-shipping">
                    ...
                    {isAdmin && <p style={{fontSize: '0.8rem', color: '#6a11cb', marginTop: '5px'}}>📞 {order.customer.phone} | ✉️ {order.customer.email}</p>}
                  </div>
                  ...
                  {!isAdmin && order.status === 'Menunggu Pembayaran' && (
                    <button className="pay-button" onClick={() => handleCustomerConfirmPayment(order.id)}>
                      Saya Sudah Bayar
                    </button>
                  )}
                  ...
                  {isAdmin && (
                    <div className="admin-actions">
                      ...
                      {order.status === 'Menunggu Verifikasi' && <button className="confirm-payment-btn" onClick={() => handleAdminVerify(order.id)}>✅ Terima & Verifikasi Pembayaran</button>}
                      {order.status === 'Menunggu Pembayaran' && <button className="confirm-payment-btn" style={{background: '#6c757d'}} onClick={() => handleAdminVerify(order.id)}>Force Confirm (Manual)</button>}
                      {(order.status === 'Diproses' || order.status === 'Diproses (COD)') && <button className="complete-btn" onClick={() => handleCompleteOrder(order.id)}>📦 Tandai Selesai & Dikirim</button>}
                      {order.status === 'Selesai' && <button className="delete-btn" onClick={() => initiateDelete(order.id)}>🗑️ Hapus Riwayat</button>}
                    </div>
                  )}
                </div>
              ))}
        ...
        <div style={{ marginTop: '20px' }}>
          {!isAdmin && (
            <button 
              ...
              style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.6)', fontSize: '0.8rem', cursor: 'pointer', textDecoration: 'underline' }}
            >
              Login Admin
            </button>
          )}
      ...
      {isLoginModalOpen && (
        <div className="modal-backdrop" onClick={() => setIsLoginModalOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <button className="modal-close-btn" onClick={() => setIsLoginModalOpen(false)}>&times;</button>
            <h3>Login Admin</h3>
            <p className="modal-note" style={{marginBottom: '20px'}}>Masukkan kredensial admin.</p>
            <input type="text" name="username" placeholder="Username" value={loginData.username} onChange={handleLoginInput} style={{ width: '100%', padding: '10px', marginBottom: '10px', border: '1px solid #cbd5e0', borderRadius: '6px', boxSizing: 'border-box' }} />
            <input type="password" name="password" placeholder="Password" value={loginData.password} onChange={handleLoginInput} style={{ width: '100%', padding: '10px', marginBottom: '20px', border: '1px solid #cbd5e0', borderRadius: '6px', boxSizing: 'border-box' }} />
            <button className="modal-close-bottom-btn" onClick={handleLoginSubmit}>Login</button>
          </div>
        </div>
      )}
    </div>
